...
public class CarreListe {
    ...
    public CarreListe(PApplet processing) {
        ...
        this.carre = new Carre(processing);
        this.hauteurGrille = processing.displayHeight / carre.size();
        this.largeurGrille = processing.displayWidth / carre.size();
    ...
    private void generatePosition(int direction) {
        if (direction == 0) {
            positionX = this.positions[numberOfSquares - 1][0] - carre.size();
            positionY = this.positions[numberOfSquares - 1][1];
        } else if (direction == 1) {
            positionX = positions[numberOfSquares - 1][0];
            positionY = positions[numberOfSquares - 1][1] + carre.size();
        } else if (direction == 2) {
            positionX = positions[numberOfSquares - 1][0] + carre.size();
    ...
    boolean outOfScreen(int positionX, int positionY) {
        if (0 + carre.size() <= positionX && positionX <= processing.displayWidth - carre.size()
                && 0 + carre.size() <= positionY && positionY <= processing.displayHeight - carre.size())
            return false;
        else
            return true;
    }
    ...
    public void newBrian() {
        positionX = positions[0][0] + carre.size() * (int) processing.random(-largeurGrille, largeurGrille);
